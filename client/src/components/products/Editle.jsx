...
const Editle = () => {
    ...
    useEffect(() => { // ürüne ait kategori alanı da seçileceği için get işlemini sağladık.
      const getCategories = async () => {
        try {
          const res = await fetch(process.env.REACT_APP_SERVER_URL + "/api/categories/get-category");
          const data = await res.json();
          data &&
            setCategories(
              data.map((item) => {
